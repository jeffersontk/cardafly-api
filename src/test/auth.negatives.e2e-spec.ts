// src/test/auth.negatives.e2e-spec.ts
...
describe('Auth (negatives) e2e', () => {
  ...
  it('GET rota protegida sem token -> 401', async () => {
    await request(app.getHttpServer()).get('/api/families/mine').expect(401);
  });
    ...
    await request(app.getHttpServer())
      ...
      .expect(res => {
        if (![401, 409].includes(res.status))
          throw new Error('esperava 401 ou 409');
      });
  });
